...
- DB: Drizzle ORM detrás de una factory en `src/db/client.ts`. Driver por defecto: **PGlite** (Postgres embebido, datos en `./.data/pglite/`, override `PGLITE_DATA_DIR` para smokes aislados); si `DATABASE_URL` está definida se usa `pg` (Postgres real). Ningún módulo importa el driver directamente — siempre `getDb()`. **Lock de proceso único** (`src/db/pglite-lock.ts`, lockfile `./.data/pglite.lock`): abrir PGlite desde un segundo proceso falla con error claro en vez de corromper (pid muerto = lock huérfano, se limpia solo); las dos vías de apertura (`getDb()` y `scripts/migrate.ts`) lo adquieren. Verificación: `npx tsx scripts/smoke-pglite-lock.ts` (aislado, se limpia solo, seguro con el dev server vivo).
